...
export default function CityMap({ cityId }: { cityId: string }) {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <Link
        href="/carte"
        className="glass absolute left-4 top-6 z-20 rounded-lg px-3 py-1.5 font-display text-[10px] uppercase tracking-wide text-accent hover:bg-white/10"
      >
        ← Retour à la carte du monde
      </Link>

      <div className="glass absolute left-4 top-16 z-20 rounded-lg px-3 py-1.5 font-display text-sm text-paper">
        {city?.name}
      </div>

      {canEdit && (
        <div className="glass-strong absolute left-4 top-28 z-20 max-w-[calc(100%-2rem)] space-y-2 rounded-2xl p-3">
