...
import { DEFAULT_BUILDING_ICONS } from '@/lib/icons';
    ...
    <div className="flex h-screen w-full flex-col overflow-hidden">
          ...
          return (
            ...
            >
              ...
              <text textAnchor="middle" dy={iconSize * 0.35} fontSize={iconSize} fill="#e7e5db">
                {DEFAULT_BUILDING_ICONS[b.type] ?? '🏗️'}
              </text>
            </g>
          );
        })}
      ...
      <MapLegend
        sections={[
          {
            title: 'Bâtiments',
            items: Object.entries(DEFAULT_BUILDING_ICONS).map(([type, icon]) => ({
              icon,
              label: BUILDING_LABELS[type as keyof typeof BUILDING_LABELS]
            }))
          },
          {
            title: 'Quartiers',
            items: districts.map((d) => ({ color: d.color, label: d.name }))
          }
        ]}
      />
