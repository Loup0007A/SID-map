        ...
        {type === 'maison' && (
          <div className="relative">
            ...
            {filteredRoster.length > 0 && !ownerId && (
              <div className="absolute z-10 mt-1 max-h-32 w-full overflow-y-auto rounded border border-parchment/20 bg-ink">
                {filteredRoster.slice(0, 6).map((p) => (
                  <button
                    ...
                    className="block w-full px-2 py-1.5 text-left text-sm hover:bg-gold/20"
                  >
                    {displayName(p)}
                  </button>
                ))}
