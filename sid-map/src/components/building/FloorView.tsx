...
export default function FloorView({
  ...
  const [drawing, setDrawing] = useState<{ start: Point } | null>(null);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          {floor.description && <p className="text-sm text-parchment/70">{floor.description}</p>}
        </div>
        <button
          onClick={() => setIs3D((v) => !v)}
          className="rounded border border-gold/40 px-3 py-1.5 text-xs text-gold hover:bg-gold/10"
        >
      ...
      <div
        className="overflow-hidden rounded border border-parchment/15 bg-[#1c1712]"
      ...
      >
        ...
        >
          ...
          {drawing && (
            <rect x={0} y={0} width={0} height={0} /> /* le rectangle final apparaît via pendingRect */
          )}
        </svg>
      </div>
